...
| 德炫人体感应器联动米家灯光 | 德炫人体感应器 → 检测到有人 | 打开米家走廊灯 |
...
### v1.3.2 (2026-06-13)
- 修复 HA 2026.6+ 兼容性问题（3个致命导入错误）
  - `homeassistant.helpers.typing` 已移除，改为 `from homeassistant.config import ConfigType`
  - `config_validation` 导入语法错误，改为 `from homeassistant.helpers import config_validation as cv`
  - `cv.empty_config_schema()` 缺少 domain 参数，已补全为 `cv.empty_config_schema(DOMAIN)`
- 更新 `manifest.json` 版本号为 1.3.2
- 修正 DALI 场景参数说明（Scene 改为 `0x09-0x18` 格式）
