...
- ✅ 支持继电器控制（Switch 实体）
- ✅ 支持 DALI 单色温灯控制（Light 实体 - 亮度调节）
- ✅ 支持 DALI 双色温灯控制（Light 实体 - 亮度和色温调节，group+1协议）
- ✅ 支持 DM2 单色温灯控制（Light 实体 - 亮度调节）
- ✅ 支持 DM2 双色温灯控制（Light 实体 - 亮度和色温调节，Class=1标准协议）
- ✅ 支持 DM2 RGBCW 灯控制（Light 实体 - 亮度/色温/RGB颜色）
- ✅ 支持窗帘控制（Cover 实体）
...
| DALI 单色温灯 | 亮度调节（DALI协议，Class=0固定） | Group (0-F), Address (00-FF) |
| DALI 双色温灯 | 亮度+色温（DALI协议，group+1技巧调色温） | Group (0-F), Address (00-FF) |
| DM2 单色温灯 | 亮度调节（DM2标准协议，Class=0） | Group (0-F), Address (00-FF) |
| DM2 双色温灯 | 亮度+色温（DM2标准协议，Class=0/1） | Group (0-F), Address (00-FF) |
| DM2 RGBCW 灯 | 亮度+色温+颜色（DM2标准协议，Class=0/1/2/3） | Group (0-F), Address (00-FF) |
...
**DALI 双色温灯：**
- 调光（同单色温）
- 调色温：`*A,0,G+1,AA;*Z,0XX;`（**注意**：DALI协议使用 group+1 技巧，Class 固定为 0）

### 灯光控制（DM2 标准协议）

DM2 调光主机使用标准 Class 参数：

| Class | 功能 | 命令格式 | 说明 |
|-------|------|---------|------|
| **0** | 亮度 | `*A,0,G,AA;*Z,0XX;` | XX=00~FF (0%~100%) |
| **1** | 色温 | `*A,1,G,AA;*Z,0XX;` | XX=00(2200K) ~ FF(6500K) |
| **2** | 色轮 | `*A,2,G,AA;*Z,0WW;` | WW=00(红) ~ FF(359°) |
| **3** | 饱和度 | `*A,3,G,AA;*Z,0SS;` | SS=00(冷白) ~ FF(纯色) |
| **4** | RGB | `*A,4,G,AA;*Z,1RR;*Z,1GG;*Z,1BB;*Z,000;` | 分段Z命令 |

**DM2 单色温灯：** 只使用 Class=0  
**DM2 双色温灯：** 使用 Class=0 (亮度) + Class=1 (色温)  
**DM2 RGBCW 灯：** 使用 Class=0/1/2/3 (亮度+色温+色轮+饱和度)
...
### v1.3.3 (2026-06-15)
- **设备类型重构**：DALI单色温/双色温与DM2系列分离为独立设备类型
  - 原「单色温灯」→「DALI 单色温灯」（Class=0固定，group+1调色温技巧）
  - 原「双色温灯」→「DALI 双色温灯」（同上）
  - 新增「DM2 单色温灯」（DM2标准Class=0协议）
  - 新增「DM2 双色温灯」（Class=0亮度 + Class=1色温，标准DM2协议）
  - 新增「DM2 RGBCW 灯」（Class=0/1/2/3，亮度+色温+色轮+饱和度）
- 反馈解析器升级：支持DM2 Class=1色温反馈和Class=2/3色轮/饱和度反馈
- 编辑设备时增加地址格式校验和交叉类型冲突检测
- 默认TCP端口从502改为2300
- 设备类型下拉菜单优化（DM2系列排在一起）
- 添加设备时验证地址格式（00-FF）和十六进制合法性
- 添加设备时全局交叉类型地址冲突检测
- 修复同连接下不同设备类型间 address 重复检测逻辑
