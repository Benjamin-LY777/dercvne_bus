...
class DALILight(DALIDevice):
    ...
    async def set_color_temp_kelvin(self, kelvin: int) -> bool:
        """Set color temperature. Subclasses (DM2 CCT) override to use Class=1.
        ...
        """
        cmd = self._encoder.set_color_temp_kelvin(
            self.group, self.address, kelvin,
        )
    ...
    async def turn_on(self, brightness: int = None, color_temp_kelvin: int = None) -> bool:
        ...
        # Call instance method so DM2 CCT subclass can override (Class=1)
        if self.supports_color_temp and color_temp_kelvin is not None and success:
            ct_result = await self.set_color_temp_kelvin(color_temp_kelvin)
            if not ct_result:
                success = False
