...
def _parse_feedback_message(raw: str) -> list:
    ...
    while i < len(segments):
        ...
        if cmd_type in ("S", "C") and len(fields) >= 4:
            ...
            # Dimmer adjust: A,class,group,addr
            # Paired with the next *Z segment: Z,0XX (3-digit hex, leading 0)
            class_ = int(fields[1])
            group = fields[2]
            addr = fields[3]
            z_value = None
            if i + 1 < len(segments):
                ...
                if z_fields[0] == "Z" and len(z_fields) >= 2:
                    try:
                        z_value = int(z_fields[1], 16)  # e.g. "019" → 25
                    except ValueError:
                        pass
        ...
        try:
            ...
            for evt in events:
                ...
                if evt_type in ("on", "off"):
                    ...
                    if key in entity_registry:
                        # Brightness feedback — group directly matches a registered entity
                        ...
                        # CT channel fallback — group = brightness_group + 1
